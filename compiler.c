	...
	case DEREF_EXPRESSION:
		...
		for (;d_index < location->data.block.expr_c;++d_index){
			...
			case USER_TYPE:
				...
				if (referenced->type.tag != STRUCT_TYPE){
					snprintf(err, ERROR_BUFFER, " [!] Cannot dereference non structure user defined type\n");
					return d_full_type;
				}
				target_struct = referenced->type.data.structure;
				break;
			default:
			...
			for (uint32_t k = 0;k<target_struct.binding_c;++k){
				...
				if (strncmp(term->data.binding.name.string, target_binding.name.string, TOKEN_MAX) == 0){
					if (target_binding.type.tag == STRUCT_TYPE){
						...
						if (nested_user_type->type.tag == STRUCT_TYPE){
							if (access_index+1<apl->data.block.expr_c){
								found = 1;
								access_full_type = nested_user_type->type;
								break;
							}
